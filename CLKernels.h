...
const char* cutoffSource = 
"__kernel void clCutoff(__global const float2* Input, __global float2* Output, int width, int height, float cutoffradius) \n"
"{		\n"
...
"		bool xquadrant = xid < (width/2); \n"
"		bool yquadrant = yid < (height/2); \n"
"		int xr = xid*(xquadrant) + (width-xid)*(!xquadrant); \n"
"		int yr = yid*(yquadrant) + (height-yid)*(!yquadrant); \n"
"		float cutoff = exp(-(xr*xr + yr*yr)/(cutoffradius*cutoffradius)); \n"
"		Output[Index].x = cutoff*Input[Index].x; \n"
"		Output[Index].y = cutoff*Input[Index].y; \n"
"	}	\n"
"}		\n"
;

const char* cutoffSource2 = 
"__kernel void clCutoff(__global const float2* Input, __global float2* Output, int width, int height, float cutoffradius) \n"
"{		\n"
"	//Get the work items ID \n"
"	int xid = get_global_id(0);	\n"
...
"		float cutoff = exp(-rad/(cutoffradius*cutoffradius)); \n"
"		Output[Index].x = cutoff*Input[Index].x; \n"
"		Output[Index].y = cutoff*Input[Index].y; \n"
"	}	\n"
"}		\n"
...
// input / (exp(abs(input2))
const char* divisionSource = 
"__kernel void clDivision(__global const float2* Input, __global const float2* Input2, __global float2* Ouput, int width, int height) \n"
"{		\n"
"	//Get the work items ID \n"
"	int xid = get_global_id(0);	\n"
"	int yid = get_global_id(1); \n"
...
"	{	\n"
"		int Index = xid + yid*width; \n"
"		Ouput[Index].x = Input[Index].x / (exp(sqrt(Input2[Index].x * Input2[Index].x + Input2[Index].y * Input2[Index].y))+0.00001); \n"
"		Ouput[Index].y = Input[Index].y / (exp(sqrt(Input2[Index].x * Input2[Index].x + Input2[Index].y * Input2[Index].y))+0.00001); \n"
"	}	\n"
"}		\n"
;

// output = log(abs(input)
const char* logAbsSource = 
"__kernel void clLogAbs(__global const float2* Input, __global float2* Ouput, int width, int height) \n"
"{		\n"
"	//Get the work items ID \n"
"	int xid = get_global_id(0);	\n"
"	int yid = get_global_id(1); \n"
...
"	{	\n"
"		int Index = xid + yid*width; \n"
"		Ouput[Index].x = log(sqrt(Input[Index].x * Input[Index].x + Input[Index].y * Input[Index].y)); \n"
"		Ouput[Index].y = 0; \n"
"	}	\n"
"}		\n"
;
